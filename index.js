...
function calculateCircleArea(radius) {
  ...
  const calc = Math.PI * Math.pow(radius, 2);
  return calc;
...
function findMinMax(numbers) {
  if (!Array.isArray(numbers)) {
    console.error("Помилка: Параметр має бути масивом чисел.");
    return null;
  }
  return {
...
function calculateHypotenuse(a, b) {
  if (typeof a !== "number" || typeof b !== "number") {
    console.error("Довжини катетів повинні бути числами");
    return null;
  }
...
function calculateVolumeCylinder(radius, height) {
  ...
  const calc = Math.PI * Math.pow(radius, 2) * height;
  const calc1 = Math.ceil(calc);
  return calc1;
...
function getFractionalPart(num) {
  ...
  const part1 = num - part;
  const part2 = Math.fround(part1);
  return part2;
...
function compareAndRound(num1, num2) {
  ...
  const mix = Math.max(num1, num2);
  const round = Math.round(mix);
  return round;
...
function estimateInvestment(principal, interestRate, years) {
  ...
  const roundedInvestmentAmount = Math.round(investmentAmount);
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  if (!Array.isArray(products)) {
    console.error("Аргумент products повинен бути масивом");
    return null;
  }
  if (typeof maxPrice !== "number") {
    console.error("Аргумент maxPrice повинен бути числом");
    return null;
  }
